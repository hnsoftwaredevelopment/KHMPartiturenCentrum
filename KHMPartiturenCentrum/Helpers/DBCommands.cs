    ...
    public static ObservableCollection<ScoreModel> GetScores ( string _table, string OrderByFieldName )
    {
        ...
        if ( dataTable.Rows.Count > 0 )
        {
            for ( int i = 0; i < dataTable.Rows.Count; i++ )
            {
                ...
                total = int.Parse ( dataTable.Rows [ i ].ItemArray [ 37 ].ToString () ) + int.Parse ( dataTable.Rows [ i ].ItemArray [ 38 ].ToString () ) + int.Parse ( dataTable.Rows [ i ].ItemArray [ 39 ].ToString () ) + int.Parse ( dataTable.Rows [ i ].ItemArray [ 40 ].ToString () );
                ...
                // If Title is empty don't add that row to the list
                if ( dataTable.Rows [ i ].ItemArray [ 6 ].ToString () != string.Empty )
                {
                    Scores.Add ( new ScoreModel
                    {
                        ...
                        NumberScoresSupplier1 = int.Parse ( dataTable.Rows [ i ].ItemArray [ 37 ].ToString () ),
                        NumberScoresSupplier2 = int.Parse ( dataTable.Rows [ i ].ItemArray [ 38 ].ToString () ),
                        NumberScoresSupplier3 = int.Parse ( dataTable.Rows [ i ].ItemArray [ 39 ].ToString () ),
                        NumberScoresSupplier4 = int.Parse ( dataTable.Rows [ i ].ItemArray [ 40 ].ToString () ),
